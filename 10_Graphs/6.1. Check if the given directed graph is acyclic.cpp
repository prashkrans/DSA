...
public:
	...
	bool hasCycleDHelper(T u, map<T, bool> recStack) {
		...
		for(auto v : adjL[u]) {
			...
			else {                              //  else if v is already visited check if it is present in the recursion stack
				if(recStack[v]==1) return 1;    //  if v is present in the recStack then a back edge exists thus a cycle exist
			}
		}
		recStack[u] = 0;    //  #IMP - Backtrack or remove the vertex u from the recursion stack which is automatically at the top since DFS(v) is completed

/*	Graph Problem #12.2.1. - Minimum Spanning Tree (MST) using modified Prim's Algorithm
	...
	2. Prim's Algorithm
		...
		2b.	Modified Prim's Algorithm (Heap of vertices) (similar to Dijkstra's Algo)
*/

//	Method 2.1 - Modified Prim's algorithm (Greedy Algo) - Better than Prim's Algo
...
//		called setPA that contains vertices yet to be included in MST
...
public:
	...
	vector<edge<T>> primsAlgoMST() {        //  Similarities from Dijkstra's Algo (label<=>dist, setPA<=>setDA, inMstSet<=>inSpSet, parent, path, cost)
		map<T, int> label;					// 	Learn it. Verified that it could not get any easier than this. [#IMP]
		...
		while(!setPA.empty()) {
			...
			inMstSet[v] = 1;	// Note: inMstSet and mstCost is used above for loop below
			mstCost += edgeWt;
			edge<T> e(edgeWt, parent[v], v); path.push_back(e);
			setPA.erase(setPA.begin());

			for(auto x : adjList[v]) {
				if(!inMstSet[x.first]) {	// Check if w is already in MST set
                    T w = x.first;
                    int wt = x.second;
                    if(wt < label[w]) {

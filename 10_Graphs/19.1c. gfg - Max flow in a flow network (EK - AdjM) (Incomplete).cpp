...
public:
    ...
    int edmondsKarpAlgo(int s, int t, vector<vector<int>> adjM, int N) {
    ...
    int solve(int N, int M, vector<vector<int>> e) {
        ...
        int ape = 0;
        map<pair<int, int>, int> apeNodes;
        for(int i=1; i<N+1; i++) {
            for(int j=1; j<N+1; j++) {
                if(i==j) adjM[i][j] = 0;                    //  self loops disallowed
                if(adjM[i][j]>0&&adjM[j][i]>0) {            //  anti-parallel edges are present
                    ape++;
                    apeNodes[{j, i}] = adjM[j][i];
                    adjM[j][i] = 0;
                }
            }
        }
        ...
        if(ape>0) {
            int newN = N + ape;
            vector<vector<int>> adjMatrix(newN+1, vector<int>(newN+1, 0));
            for(int i=1; i<N+1; i++) {
                for(int j=1; j<N+1; j++) {
            ...
            printMat(adjM, N);
            return edmondsKarpAlgo(1, newN, adjMatrix, newN);
        }
        printMat(adjM, N);
        return edmondsKarpAlgo(1, N, adjM, N);

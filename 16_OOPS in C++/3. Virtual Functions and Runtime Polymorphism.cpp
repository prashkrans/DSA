...
This happens due to static linkage which means the call to foo() is getting set only once by the compiler which is in the base class.
...
b.	vptr: A pointer to vtables per class, maintained per object instance.
...
2.)	Code with polymorphic function call (e.g. bp->show() in above code). Wherever a polymorphic call is made, the compiler inserts code to 
	...
	of derived class is accessed). Once vptr of required vtable is fetched, vtable of derived class can be accessed. Using vtable, address of 
	derived class function foo() is accessed and called. 
...
functions of the derived classes that override the member functions of the base class. It is not mandatory but using it minimizes bugs and errors.

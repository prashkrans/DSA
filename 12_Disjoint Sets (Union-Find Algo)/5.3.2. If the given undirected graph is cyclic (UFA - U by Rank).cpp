...
//  Here, we are using union by rank without path compression to use the union-find algorithm
...
public:
	...
	pair<T, int> findSet(T u, unordered_map<T, pair<T, int>> subset) {
		T parent = subset[u].first;
		while(parent!=u) {
			u = parent;
			parent = subset[u].first;
		}
		return subset[u];	//  returns the root of the set in which u was present
	}

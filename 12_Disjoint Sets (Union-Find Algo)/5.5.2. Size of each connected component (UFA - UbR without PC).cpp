/*Graph Problem #8.5.2. Size of each connected component
...
//	Method 1.2. Using union-find algo (Union by rank without path compression)
//	Running time = O(m*(2*Find() + Union()) = O(m*(2*log2(n) + log2(n))) ~ O(mlog2(n)). Since, for each of the m edges we run two find() and one union() operations
...
pair<T, int> findSet(T u, unordered_map<T, pair<T, int>> &subset) { //  It is a recursive function that first finds the root of the set in which u is present
		T parent = subset[u].first;
		int rank = subset[u].second;
		if(parent == u) return {u, rank};       					//  returns the root of the set in which u is present
		return findSet(parent, subset);                  			//  returns the root and rank after the path compression
	}

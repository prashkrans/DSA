/*	Graph Problem #8.4.1. - Given an undirected graph, find whether it has a cycle. [Verify once although seems correct]
...
//	Method 3.2. Using the union-find algorithm (Union by rank or height i.e. find() = O(log2(n)) and union() = O(log2(n)))
//  Here, we are using union by height without path compression to use the union-find algorithm
...
//	Running time = O((m*2*Find()) + (Union()*(n-1))) = O(2mF + U(n-1)) ~ O(mF + Un) = O(mlog2(n) + nlog2(m)) ~ O(n^2log2(n) + 2nlog2(n)) ~ O(n^2*log2(n)) or O(mlog2(n)) if m = n^2
...
public:
	...
	T findSet(T u, unordered_map<T, pair<T, int>> &subset) {            //  It is a recursive function that first finds the root of the set and makes it the parent
		...
		//else subset[u].first = findSet(parent, subset);               //  Path compression i.e. makes root the parent of all the nodes in the current path (Not used here)
		return findSet(parent, subset);	                                //  returns the root after the path compression
	}
	void unionOfSets(T u, T v, unordered_map<T, pair<T, int>> &subset) {    //  Union by height is slightly different but easier than union by rank
		T uRoot = findSet(u, subset);
		T vRoot = findSet(v, subset);

/* Queue problem - 5. LC#286. Walls and Gates
...
//	Note, here the visited array is not required as the if(rooms[vi][vj] > 1 + rooms[ui][uj]) => v is not yet visited from the nearest gate
    ...
    void wallsAndGates(vector<vector<int>> &rooms) {
        ...
        while(!Q.empty()) {
            ...
            for(int i=0; i<4; i++) {
                ...
                if(isValid(v, m, n) && rooms[v.first][v.second] > 1 + rooms[u.first][u.second]) {  //	If (isValid(v) and until v is not visited from the nearest gate)
                    Q.push(v);

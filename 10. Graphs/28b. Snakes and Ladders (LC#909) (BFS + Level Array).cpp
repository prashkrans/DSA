/* Graph problem 31b. LC#909. Snakes and Ladders
...
*/
//	Method 1. Simple BFS + Level Array
//	Steps: 
...
//	2.	Use a simple BFS to find the shortest path from source node == 1 to destination node == n^2 as the flat board can be considered
...
public:
    ...
    int snakesAndLadders(vector<vector<int>>& board) {	//	Simple BFS to find the shortest distance in an unweighted graph
        ...
        while(!Q.empty()) {
            int u = Q.front();
            Q.pop();
            //if(u==V) return level[u];					//	If the curr node u is the last node V, then return minMoves which is the shortest dist. between 1 and V
            for(int j = u+1; j <= min(u+6, V); j++) {	//	For each node u, we have 6 adjacent nodes
                ...
                else if(flatBoard[j]!=-1) v = flatBoard[j];	//	Else if fB[j]==-1 => jth cell has a snake or a ladder => v = fB[j]; [#IMP] Here, we don't add j instead add fB[j] as an adjacent node of u
        ...
        //for(int i=1; i<=V; i++) cout<<level[i]<<"\t"; cout<<"\n";

/* Graph Problem 26. LC#542. 01 Matrix
Given an m x n binary matrix mat, return the distance of the nearest 0 for each cell.
The distance between two adjacent cells is 1.
The method used to solve this can be exactly used in the problem LC#286. Walls and Gates
...
//	Method 3. BFS (2 passes - First for finding all the 0s and second for the BFS)
//	We can use Breadth First Search(BFS) to limit the search to the nearest 0 found for each 1. As soon as a 0 appears during
//	the BFS, we know that the 0 is nearest, and hence, we move to the next 1.
...
//	Algorithm:
//	a.	For our BFS routine, we keep a queue, q to maintain the queue of cells to be examined next.
//	b.	We start by adding all the cells with 0s to q.
//	c.	Intially, distance for each 0 cell is 0 and distance for each 1 is INT_MAX, which is updated during the BFS.
//	d.	Pop the cell from queue, and examine its neighbours. If the new calculated distance for neighbour {i,j} is smaller, we add {i,j} to q and update dist[i][j].

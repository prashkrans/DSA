/* DP-Problem 6b (Uses the 0th row and column) - Only minor changes like indexes changed in d[]+d[]+d[] and k = k+1.
...
int matrixChainMultiplication2(vector<int> d, int n){   //  Extra row and column not used
	vector<vector<int>> cost(n, vector<int>(n));  	    //	0th row and 0th column of cost and kVal are used and hence (i,j) begins from (0,0)
	vector<vector<int>> kVal(n, vector<int>(n));        //  The DP table is named cost & kVal table is only used only when we need to print the order of parenthesization
	...
	for(int diff = 1; diff<n; diff++){                  //  Using the difference between to i and j to iterate diagonally in the DP table. It varies between 1 to n-1
		i = 0;                                          //  i is made 1 again after all the entries in a diagonal is calcd to move to the next diagonal
		...
		while(j<n){
			...
			for(int k = i; k<j; k++){                   //  Three nested loops hence the time required is O(n^3)
				int kCost = cost[i][k] + cost[k+1][j] + d[i]*d[k+1]*d[j+1]; //  Indexes used in d[]+d[]+d[] changed here as (i,j) begins from (0,0)
				if(kCost<minVal) {
					minVal = kCost;
					kMin = k+1;                         // kmin = k+1 as (i,j) begins from (0,0)
                }
			}
			cost[i][j] = minVal;
			kVal[i][j] = kMin;
	...
	cout<<"kVal Table = \n";
    for(int i=0; i<n; i++){
        for(int j=0; j<n; j++) cout<<kVal[i][j]<<" ";
        cout<<"\n";
	}
	return cost[0][n-1];                                  // The final result is stored in the last column of the first row
}
...
int main(){
	...
	d[3] = 2;
	d[4] = 5;
    int minCost = matrixChainMultiplication2(d, n);
    ...
    while(input=='y'){
        ...
        minCost = matrixChainMultiplication2(d2, n);

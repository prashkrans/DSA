...
//  This solution is easier to understand while the second one uses custom comparator instead of greater<dataType>()
...
public:
    int maximumUnits(vector<vector<int>>& bT, int tS) {
        ...
        // sort(arr.begin(), arr.end(), greater<dataType>()); for descending order
...
//  Method 3. Using cmp class outside of the class Solution and using a for loop:
//	Running time = O(nlogn) due to sorting of the given vector of size n (rows: n cols: 2).
//	Auxiliary space = O(1) i.e. constant as we are sorting the given 2D vector itself instead of creating a new 2D vector
class cmp {
    public:
    ...
    public:
        int maximumUnits(vector<vector<int>>& boxTypes, int truckSize) {
            ...
            for(int i=0; i<boxTypes.size(); i++) {
                ...
                int currUnits = boxTypes[i][1];            
                if(truckSize < currBoxes) currBoxes = truckSize;
                maximumUnits += currBoxes * currUnits;
                truckSize -= currBoxes;
            }
            return maximumUnits;
        }

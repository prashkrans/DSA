...
//	Method 1 - Using a hashmap of stones
//	Running time = O(m+n)
//	Auxiliary space = O(n);
...
//	Method 2 - Using a hashset of jewels
//	Running time = O(m+n) | 0ms solution beats 100%
//	Auxiliary space = O(n);
...
class Solution {
    public:
        int numJewelsInStones(string jewels, string stones) {
            ...
            for (auto i: stones) if(st.find(i) != st.end()) count++;
            return count;
        }
};
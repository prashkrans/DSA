...
bool search(node *root, int data) {				//	BST Search
...
// TreeNode* searchBST(TreeNode* root, int val) { // LC# 700
...
node* deleteNode(node *root, int data) {
    ...
    if(root->val > data) root->left = deleteNode(root->left, data);
        ...
        if(root->left==NULL && root->right == NULL) {                   //  Case 1. x has no children
            ...
            delete child;
        }                                                               //  Successor could be either right or left subtree. a. If succ = rightST traverse to the leftmost node of the rightST and b. vice-versa
            ...
            // Or we could swap the values of root-> val and successor->val and call deleteNode(root->right, data)
        }
    }

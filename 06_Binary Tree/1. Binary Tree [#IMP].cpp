...
/* 10.2. Invert a binary tree	// Note the use of temp pointer to swap left and right sub trees
...
// Refer 1b_Binary_Tree_Level_Order_Traversal_LC#102.cpp
// 15c. LC#102 A leetcode problem which is a variation of 15a. Given a binary tree, return the level order traversal of its nodes'
...
public:
    ...
    return bfsArr;dsa_notes_mar_2025/2. DSA - Topic Wise/6. Binary Tree/1b_Binary_Tree_Level_Order_Traversal_LC#102.cpp
    }
};
*/
...
// 18.  (LC#101) - Write a function isSymmetric() to check whether the given binary tree is symmetric around its center i.e. the left
...
bool isSymmetricIter(node* root) {                      // leetcode soln
        ...
        while(!Q.empty()){
            ...
            Q.push(root1->left);						//	root1->left == root2->right and root1->right == root2->left
            Q.push(root2->right);
            Q.push(root1->right);
            Q.push(root2->left);
        }

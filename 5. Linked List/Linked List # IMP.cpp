...
//      i.e. without using the length() function # IMPORTANT
void frontBackSplitFastSlowptr(node*& head, node*& head1, node*& head2){
    if(head==NULL || head->next==NULL){             // Takes care of cases with length < 2 without using the length func
        ...
        while(fast!=NULL){
            ...
            if(fast!=NULL){
                slow = slow->next;                  // for each 2 advances of fast slow advances once
...
void moveNode(node*& destHead, node*& sourceHead){  // Please make sure to use *& and not just * as this problem
    if(sourceHead == NULL) return;                  // just wasted half an hour of life just because I did not use *&
    else {
...
//      the same, add a node to the result. Otherwise, advance whichever list is smaller
...
// 25. Write a function swapPairs() to swap pairs in a given linked list as demonstrated. (Recursive) Eg. {1, 2, 3, 4, 5} becomes {2, 1, 4, 3, 5}
...
/*  25b. It swaps node values instead of nodes itself, recursive
class Solution {
public:
